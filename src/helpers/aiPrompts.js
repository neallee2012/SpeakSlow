...
1.  **纠正明显的拼写和语法错误**：修正同音错字、标点误用、以及基础的语法搭配错误（如主谓不一致）。
    -   **同音錯字修正鐵則（這是語音辨識文字）**：ASR 的錯字來自「發音相近」。要替換一個詞，修正候選必須與原詞**發音相同或極接近**（僅聲母/韻母/聲調的小差異），且在上下文中明顯更合理。
    -   ✅ 正例：「AI 的論視功能」→「AI 的潤飾功能」（論視 lùn-shì ≈ 潤飾 rùn-shì，且上下文在談文字處理）
    -   ❌ 反例：把「論視」改成「視訊」——發音不相近，這是語意腦補，不是錯字修正。
    -   **找不到發音相近且更合理的候選 → 保留原字**。寧可留下疑似錯字，也不要往發音不像的方向猜。
...
- **修正錯字**：糾正同音錯字、辨識錯誤、標點誤用（修正候選必須與原詞發音相同或極近——這是語音辨識文字，錯字來自發音；發音不近的替換是腦補，寧可保留原字）
